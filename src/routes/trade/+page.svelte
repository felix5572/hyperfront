<script lang="ts">
	...
	async function fetchUserAbstraction(user: `0x${string}`): Promise<string | null> {
		...
		if (!res.ok) throw new Error(`userAbstraction failed: ${res.status} ${res.statusText}`);
...
<div class="flex flex-col h-full overflow-y-auto">
	...
	{#if showInstallTips}
		<div class="px-4 pb-2">
			<div class="p-3 rounded-lg border border-border-secondary bg-surface-secondary space-y-2">
				<div class="flex items-center justify-between">
					<h3 class="text-xs font-semibold">Install Hyperfront App (PWA)</h3>
					{#if isStandalone}
						<span class="text-[10px] px-1.5 py-0.5 rounded bg-long-dim text-long">Installed</span>
					{/if}
				</div>

				{#if !isStandalone && installPromptEvent}
					<p class="text-[11px] text-gray-500">
						Use the button below to install Hyperfront to your device home screen.
					</p>
					<button
						...
						disabled={installing}
						onclick={installApp}
					>{installing ? 'Installing…' : 'Install Hyperfront App'}</button>
				{:else if !isStandalone && isIOS}
					<p class="text-[11px] text-gray-500">
						iOS Safari manual install: tap Share, then choose “Add to Home Screen”.
					</p>
				{:else if !isStandalone}
					<p class="text-[11px] text-gray-500">
						If install button is unavailable, open browser menu and choose “Install app” / “Add to Home Screen”.
					</p>
				{/if}
	...
	<div class="px-4 pb-2">
		<div class="p-3 rounded-lg border border-amber-200 bg-amber-50">
			<h3 class="text-xs font-semibold text-amber-800 mb-1">Disclaimer</h3>
			<p class="text-[11px] text-amber-900/80">
				Hyperfront is an unofficial community frontend built around the Hyperliquid ecosystem. It is not created,
				maintained, or endorsed by the official Hyperliquid development team. Use at your own risk. Never share
	...
	<div class="px-4 pb-2">
		<div class="p-3 rounded-lg border border-border-secondary bg-surface-secondary">
			<div class="flex items-center justify-between mb-1">
				...
				<span class="text-[10px] text-gray-500">{formatTime($subscriptionStatus.updatedAt)}</span>
			</div>
			<div class="grid grid-cols-2 gap-2 text-[11px]">
				<div>
					<p class="text-gray-500">Endpoint</p>
					<p class="font-mono break-all">{$subscriptionStatus.endpoint}</p>
				</div>
				<div>
					<p class="text-gray-500">Active Subscriptions</p>
					<p class="font-semibold tabular-nums">{$subscriptionStatus.activeCount}</p>
				</div>
				<div>
					<p class="text-gray-500">Unique User Streams</p>
					<p class="font-semibold tabular-nums">{$subscriptionStatus.uniqueUserCount}</p>
				</div>
				<div>
			...
			{#if $subscriptionStatus.lastError}
				<p class="mt-2 text-xs text-red-600 break-words">{$subscriptionStatus.lastError}</p>
			{/if}
			<details class="mt-2">
				<summary class="text-[11px] text-gray-600 cursor-pointer">Active Keys ({$subscriptionStatus.activeKeys.length})</summary>
				<div class="mt-1 max-h-28 overflow-y-auto space-y-0.5">
					{#if $subscriptionStatus.activeKeys.length === 0}
						<p class="text-[11px] text-gray-500">No active subscriptions</p>
					{:else}
						{#each $subscriptionStatus.activeKeys as key (key)}
							<p class="text-[10px] font-mono text-gray-600 break-all">{key}</p>
						{/each}
					{/if}
				</div>
			</details>
	...
	{:else if loading}
		<div class="flex-1 flex items-center justify-center p-8 text-sm text-gray-500">Loading user info...</div>
	{:else}
		<div class="px-4 pb-3">
			<div class="flex items-center justify-between">
		...
		<div class="px-4 pb-4 space-y-3">
			...
			<div class="p-3 rounded-lg border border-border-secondary bg-surface-secondary">
				<h3 class="text-xs font-semibold mb-2">Sub Accounts ({subAccounts.length})</h3>
				{#if subAccounts.length === 0}
					<p class="text-xs text-gray-500">No subaccounts</p>
				{:else}
					<div class="space-y-2">
						{#each subAccounts as sub (sub.subAccountUser)}
							<div class="p-2 rounded border border-border-secondary bg-surface-tertiary/50">
								<div class="text-xs font-medium">{sub.name || 'Unnamed'}</div>
								<div class="text-[11px] text-gray-500 font-mono break-all mt-0.5">{sub.subAccountUser}</div>
								<div class="mt-1 flex flex-wrap gap-2 text-[11px] text-gray-600">
									<span>Account Value {formatUsd(parseFloat(sub.clearinghouseState?.marginSummary?.accountValue ?? '0'))}</span>
									<span>Withdrawable {formatUsd(parseFloat(sub.clearinghouseState?.withdrawable ?? '0'))}</span>
									<span>USDC {formatUsd(parseFloat(getUsdBalance(sub)))}</span>
								</div>
							</div>
						{/each}
					</div>
			...
			<div class="p-3 rounded-lg border border-border-secondary bg-surface-secondary">
				...
				<div class="space-y-1 text-[11px]">
					<p>Referred By: <span class="font-mono">{referral?.referredBy?.referrer ?? 'N/A'}</span></p>
					<p>Code: <span class="font-mono">{referral?.referredBy?.code ?? 'N/A'}</span></p>
					<p>Cumulative Volume: {formatUsd(parseFloat(referral?.cumVlm ?? '0'))}</p>
					<p>Unclaimed Rewards: {formatUsd(parseFloat(referral?.unclaimedRewards ?? '0'))}</p>
					<p>Claimed Rewards: {formatUsd(parseFloat(referral?.claimedRewards ?? '0'))}</p>
					<p>Builder Rewards: {formatUsd(parseFloat(referral?.builderRewards ?? '0'))}</p>
				</div>
				{#if referral?.referrerState?.data?.referralStates?.length}
					<div class="mt-2 border-t border-border-secondary pt-2">
						<p class="text-[11px] text-gray-500 mb-1">Referral Users ({referral.referrerState.data.referralStates.length})</p>
						<div class="space-y-1">
							{#each referral.referrerState.data.referralStates.slice(0, 5) as s (s.user)}
								<div class="text-[11px]">
									<span class="font-mono">{s.user}</span>
									<span class="text-gray-500"> · Joined {formatTime(s.timeJoined)} · Vlm {formatUsd(parseFloat(s.cumVlm ?? '0'))}</span>
								</div>
							{/each}
						</div>
					</div>
			...
			<div class="p-3 rounded-lg border border-border-secondary bg-surface-secondary">
				...
				<div class="grid grid-cols-2 gap-2 text-[11px]">
					...
					<div>User Add Rate: <span class="font-mono">{userFees?.userAddRate ?? 'N/A'}</span></div>
					<div>User Spot Cross: <span class="font-mono">{userFees?.userSpotCrossRate ?? 'N/A'}</span></div>
					<div>User Spot Add: <span class="font-mono">{userFees?.userSpotAddRate ?? 'N/A'}</span></div>
					<div>Referral Discount: <span class="font-mono">{userFees?.activeReferralDiscount ?? '0'}</span></div>
					<div>Staking Discount: <span class="font-mono">{userFees?.activeStakingDiscount?.discount ?? '0'}</span></div>
				</div>
			</div>

			<details class="p-3 rounded-lg border border-border-secondary bg-surface-secondary">
				<summary class="text-xs font-semibold cursor-pointer">Raw JSON</summary>
				<div class="mt-2 space-y-2">
					<details>
						<summary class="text-[11px] cursor-pointer">subAccounts</summary>
						<pre class="mt-1 text-[10px] overflow-x-auto">{JSON.stringify(rawSubAccounts, null, 2)}</pre>
					</details>
					<details>
						<summary class="text-[11px] cursor-pointer">referral</summary>
						<pre class="mt-1 text-[10px] overflow-x-auto">{JSON.stringify(rawReferral, null, 2)}</pre>
					</details>
					<details>
					...
					<details>
						...
						<pre class="mt-1 text-[10px] overflow-x-auto">{JSON.stringify(rawAbstraction, null, 2)}</pre>
					</details>
				</div>
			</details>
		</div>
